...
inherit meson multilib-minimal flag-o-matic cros-toolchain-funcs cros-workon arc-build
...
#	newexe "${BUILD_DIR}/src/mapi/shared-glapi/libglapi.so.0" libglapi.so.0
	...
	exeinto "${ARC_PREFIX}/vendor/$(get_libdir)"
	doexe "${BUILD_DIR}/src/gallium/targets/dri/libgallium_dri.so"

	if use vulkan; then
		exeinto "${ARC_PREFIX}/vendor/$(get_libdir)/hw"
		newexe "${BUILD_DIR}/src/panfrost/vulkan/libvulkan_panfrost.so" vulkan.cheets.so
	fi
}
